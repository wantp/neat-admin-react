...
import {getMenuPaths,loopMenuItem} from '@/utils/menu';
import {queryCurrent, queryCurrentMenus} from './services/current';
...
export async function getInitialState(): Promise<{
  ...
  if (getAccessToken()) {
    ...
    let menuPaths;
    if (currentUser) {
      menuData = await queryCurrentMenus();
      menuPaths = getMenuPaths(menuData);
    }
    return {
      fetchUserInfo,
      currentUser,
...
export const layout: RunTimeLayoutConfig = ({initialState}) => {
  return {
    ...
    onPageChange: () => {
      ...
      // 如果已登录，登录页重定向到首页
      if (location.pathname === '/user/login' && initialState?.currentUser) {
        history.push('/');
      }
    },
...
const headerInterceptor = (url: string, options: RequestOptionsInit) => {
  const headers = {
